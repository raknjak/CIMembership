 ...
 *
 * ATTENTION: THIS FILE HAS BEEN MODIFIED TO WORK WITH CIMEMBERSHIP
 *
 * @category	Libraries
...
class Cache
{
	...
	private function _call($property, $method, $arguments = array(), $expires = NULL)
	{
		...
		$cache_file = $property . DIRECTORY_SEPARATOR . hash($method . base64_encode(serialize($arguments)), 'sha1');
	...
	function get($filename = NULL, $use_expires = true)
	{
		...
		if (filesize($filepath) > 0)
		{
            ...
            // CIM: fix unserialize offset error
            // https://stackoverflow.com/questions/10152904/unserialize-function-unserialize-error-at-offset#10152996
            ...
            $data = $this->_repairSerializeString($data);

            $this->_contents = unserialize(base64_decode($data));
	...
	function write($contents = NULL, $filename = NULL, $expires = NULL, $dependencies = array())
	{
		...
		if (flock($fp, LOCK_EX))
		{
			fwrite($fp, base64_encode(serialize($this->_contents)));
			flock($fp, LOCK_UN);
		}
		else
		{
	...
	// CIM
    private function _repairSerializeString($bad_data)
    {
